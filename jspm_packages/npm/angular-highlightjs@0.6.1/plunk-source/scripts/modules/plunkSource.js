/* */ 
(function(process) {
  angular.module('plunkSource', ['ngSelect', 'hljs']).factory('_apiPlunkData', function($http) {
    return function(plunkId) {
      var api = "http://api.plnkr.co/plunks/",
          config = {cache: true};
      return $http.get(api + plunkId, config).then(function(res) {
        return res.data;
      });
    };
      ...
      link: function(scope, iElm, iAttrs) {
        ...
        scope.$watch('plunkId', function(newId, oldId) {
          if (angular.isString(newId) && newId) {
            ...
            _apiPlunkData(newId).then(function(data) {
