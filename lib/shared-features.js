...
//var test = selection.select("foo").attrs({"bar":"baz"});
...
export function createPlotControls(root, controls, activeControls = []) {
    ...
    controlPanel.selectAll("i")
        .data(ctrls)
        .enter()
        .append("i")
        .attr("class", action => `plot-control fa ${ICONS[action]} action-${action}`);
    ...
    plotRoot.select("i.action-zoom")
        ...
        .on('click', function() {
            ...
            let selectMode = plotRoot.select("i.action-select");
    ...
    plotRoot.select("i.action-select")
        ...
        .on('click', function() {
            ...
            self.classed("active", !active);
            let zoomMode = plotRoot.select("i.action-zoom");
    ...
    plotRoot.select("i.action-label")
    ...
    plotRoot.select("i.action-download")
        .on('click', function(){
...
export class LabelCollisionDetection {
    ...
    quadtreeSearchWithTransform(point, {x: tx, y: ty, k: k} = {x: 0, y: 0, k: 1}) {
        ...
        this.quadtree.visit((node, x1, y1, x2, y2) => {
            ...
            let p = node.data;
            if (p) {
                if(this.dist(px, py, p.x * k + tx, p.y * k + ty) <= r && p != point) foundNodes.push(p);
            }
            return false;
        });
