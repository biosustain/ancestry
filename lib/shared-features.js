...
export function getClipUniqueId() {
    return d3.max(d3.selectAll('clipPath').data()) + 1;
}
...
export function createPlotControls(root, controls, activeControls = new Set()) {
    const ICONS = {
    ...
    plotRoot.select('div.action-brush')
        .classed("active", activeControls.has('brush'))
        .on('click', function() {
            let self = d3.select(this);
            let active = self.classed("active");
            (controls['brush'])(!active);
            self.classed("active", !active);
            setActiveControl();
        });
...
export function skipProperty(obj, prop) {
...
export class LabelCollisionDetection {
    constructor(levelFixed, levels, nodeLabelPositions, width, height, markerBBox) {
        this.width = width;
        this.height = height;
        this.levels = levels;
    ...
    recalculateLabels() {
        ...
        let isVisible = d => {
            return d.x >= -10 && d.x <= this.width + 10 && d.y >= -10 && d.y <= this.height + 10;
        };
...
let colorBarID = 0;
export function drawColorBar(selection, domain, heatmapOptions, defs, defsRoutePath) {
    ...
    selection.append("rect")
        ...
        .attr("width", width)
        .attr("height", height)
        .style("fill", `url(${defsRoutePath}#gradient${colorBarID++})`)
        .attr("stroke-width", 2)
        .attr("stroke", "grey")
...
export function filterSeries(nodes, activeSeries) {
    let filteredNodes = [],
        nodesDict = {},
        parent;
