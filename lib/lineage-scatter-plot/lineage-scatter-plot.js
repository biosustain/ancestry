...
import {  d3legend, d3tooltip, mergeTemplateLayout, createNodeTypes, createDynamicNodeAttr, roundOffFix, LabelCollisionDetection,
    ...
    multiAttr, getTranslation, createPlotControls} from '../shared-features.js'
            ...
            function render(options) {
                ...
                let circle = node.append("circle")
                    ...
                    .on("mouseover", function(d) {
                        let {x: xPos, y: yPos} = d3tooltip.getRelativePosition(this, element[0]),
                            text = `<div class="tooltip-colour-box" style=\"background-color: ${colours(d.data.series)}\"></div>` +
                                ...
                                `<span class="tooltip-text">x: ${d.data.x.toPrecision(3)}</span>` +
                                `<span class="tooltip-text">y: ${d.data.y.toPrecision(3)}</span>`;
                    ...
                    circle.on("mouseover", function (d, i) {
                        let {x: xPos, y: yPos} = d3tooltip.getRelativePosition(this, element[0]),
                            seriesBar = layout.tooltip.showSeriesBar ?
                                `<div class="tooltip-colour-box" style=\"background-color: ${colours(d.data.series)}\"></div>` : "",
                            text = d.data.tooltip ? d.data.tooltip.map((line) => `<span class="tooltip-text">${line}</span>`).join("") :
                                `<span class="tooltip-text">${d.data.name}</span>`;
                ...
                function click(d) {
                    ...
                    if (!n.classed("selected")) {
                        ...
                        n.select("circle").style("fill", d => colours(d.data.series));
                    }
                    else {
...
export default angular.module('ancestry.lineage-scatter', ['ancestry.utils'])
