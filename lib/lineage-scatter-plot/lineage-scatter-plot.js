...
class LineageScatterPlotController {
    ...
    initializeData(value, options = {isNewData: true}) {
        ...
        this.descendants = this.prepareNodes(data, this.plotWidth, this.plotHeight);
        ...
        if (!options.isNewData) {
            ...
            this.selectedNodesSet = new Set();
            this.descendants.filter(d => d.data.selected).forEach(d => {
        ...
        this.xExtent = d3.extent(this.descendants, node => node._x);
        this.yExtent = d3.extent(this.descendants, node => node._y);
        ...
        this.yScale = d3.scaleLinear()
            .domain(this.yExtent.slice().reverse())
    ...
    adjustScale() {
        ...
        if (paddingX == null || paddingY == null) {
            let longestNodeName = this.descendants.length ? this.descendants.reduce((a, b) =>
                ...
                maxLabelSize = testLabelLength(this.svg, longestNodeName, this.layout.nodeLabel),
                maxLabelOffset = {
                    x: d3.max(this.layout.nodeLabelPositions, (pos) => Math.abs(pos.x)),
                    y: d3.max(this.layout.nodeLabelPositions, (pos) => Math.abs(pos.y))
                };
            if (paddingX == null) {
                paddingX = maxLabelSize.width + maxLabelOffset.x + 5;
            }
            if (paddingY == null) {
                paddingY = maxLabelSize.height + maxLabelOffset.y + 5;
            }
        }
    ...
    drawAxis(container, type, scale, tickLength, isTimeAxis=false, fixTicks=false) {
        ...
        let axis = (axisType == 'x' ? d3.axisBottom : d3.axisLeft)()
            .scale(scale)
            .tickSizeInner(0)
            .tickSizeOuter(0);
        ...
        this.axisSvgs[axisSvg].call(axis);
        LineageScatterPlotController.adjustAxisStyles(this.axisSvgs[axisSvg], layout);
    }
    ...
    drawLinks(redraw = true) {
        if (redraw) {
            this.link = this.treeContainer.append('g')
                ...
                .insert('path', 'g')
                .attr('class', 'link')
                .attr('marker-end', `url(${this._$window.location.pathname}#marker-arrowhead)`)
        ...
        this.link.attr('d', LineageScatterPlotController.linkGenerator);
    ...
    drawColorBar() {
        if (this.layout.heatmap.enabled && this.layout.heatmap.colorBar.show) {
    ...
    makeLCD() {
        if (this.layout.labelCollisionDetection.enabled === 'onEveryChange' ||
            ...
            this.LCD = new LabelCollisionDetection([this.circle], order, this.layout.nodeLabelPositions,
                this.plotWidth, this.plotHeight, this.markerBBoxes);
    ...
    makeZoom() {
        this.zoom = d3.xyzoom()
            ...
            .translateExtent([[0, 0], [this.plotWidth, this.plotHeight]])
            .on('zoom', this.onZoom);
    }
    ...
    makeBrush() {
        ...
        let brushDescendants = this.prepareNodes(this.lastData, this.plotWidth, this.plotHeight),
        ...
        if (this.layout.brush.drawTrees) {
            this.brushContainer.append('g')
                ...
                .insert('path', 'g')
                .attr('class', 'link')
                .attr('d', LineageScatterPlotController.linkGenerator)
                .attrs(this.layout.link);
