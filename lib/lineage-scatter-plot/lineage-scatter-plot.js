        ...
        link(scope, element, attributes) {
            ...
            function render(options) {
                ...
                if (scope.selected.length)
                    nodesData = data.nodes.filter((node) => {
                ...
                let xAxisFormat = isTimePlot ? d3.time.format(data.xAxis.format || defaultTimeFormat) :
                                  d3.format(data.xAxis.format || defaultScalarFormat),
                ...
                let xAxisLabel = data.xAxis.title;
                let yAxisLabel = data.yAxis.title;
            ...
            function createLinks() {
                ...
                let nodeDict = {};
                links = [];
                for(let node of scope.value.nodes) {
                    node.children = [];
                ...
                let nodes = scope.selected.length ? scope.selected : scope.value.nodes.map(node => node.name);
