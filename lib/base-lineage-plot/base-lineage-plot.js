...
class BaseLineagePlotController {
    ...
    makeBrush() {
        ...
        this.brushContext.plotHeight = this.layout.brush.height;
        this.brushContext.plotWidth = this.isCartesian ? this.plotWidth : this.plotWidth / (this.plotHeight /
            this.layout.brush.height);
        this.brushContext.xScale = this.xScale.copy().range([0, this.brushContext.plotWidth]);
        this.brushContext.yScale = this.yScale.copy().range([0, this.brushContext.plotHeight]);
        this.updatePositions(this.brushContext);
        this.brushContext.plotOrigin = this.originAtCenter ? [this.brushContext.plotWidth / 2,
            this.brushContext.plotHeight / 2] : [0, 0];

        let fullExtent = [[0, 0], [this.brushContext.plotWidth, this.brushContext.plotHeight]],
        ...
        this.defs.append('svg:clipPath')
            ...
            .attrs({
                x: -1,
                y: -1,
                width: this.brushContext.plotWidth + 3,
                height: this.brushContext.plotHeight + 3
            })
        ...
        this.brushFixedContainer = this.svg.append('g')
            .attr('transform', `translate(${this.margin.left + (this.isCartesian ? 0 : this.plotWidth / 2 -
                this.brushContext.plotWidth / 2)}, ${this.margin.top + this.plotHeight + brushMarginTop})`);
        ...
        if (this.layout.brush.drawTrees) {
            this.brushContainer.append('g')
                .attr('class', 'link-layer')
                .selectAll('path.link')
                .data(this.brushContext.nodes.filter(n => n.parent.data.name != 'virtualRoot'))
                .enter()
                .insert('path', 'g')
                .attr('class', 'link')
                .attr('marker-end', this.layout.showLinkArrowhead ?
        ...
        this.brushFixedContainer.append('rect')
            ...
            .attr('x', -1)
            .attr('y', -1)
            .attr('width', this.brushContext.plotWidth + 2)
            .attr('height', this.brushContext.plotHeight + 2)
            .attrs(this.layout.brush.boxRectangle);
        ...
        function brushed() {
            ...
            let sx = s != null ? (lockY ? [s[0], s[1]] : [s[0][0], s[1][0]]) : that.brushContext.xScale.range(),
                sy = s != null ? (lockY ? [0, that.brushContext.plotHeight] : [s[0][1], s[1][1]]) : that.brushContext.yScale.range(),
                ...
                newTransform = d3.xyzoomIdentity
                    .scale(kx, ky)
                    .translate(-that._xScale(that.brushContext.xScale.invert(sx[0])), -that._yScale(that.brushContext.yScale.invert(sy[0])));
                                // TODO: -sx[0]?
