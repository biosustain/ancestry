...
class LineagePlotController {
    ...
    render(options) {
        ...
        console.log(this.element[0], this.element[0].offsetWidth);
        let elementWidth = this.element[0].offsetWidth,
            elementHeight = this.element[0].offsetHeight;
        console.log(this.element[0].getBoundingClientRect(),'xy')
        ...
        let node = treesContainer.selectAll("g.node")
            ...
            .enter().append("g")
            .attr("class", "node")
            .classed("selected", (d) => this.selectedNodes.has(d.data.name))
            .attr("transform", d => `translate(${d.x},${d.y})`);
        // Add node circles
        let circle = node.append("circle")
            .attr("class", "node-circle")
            .style("fill", d => !this.selectedNodes.has(d.data.name) ? '#FFF' : this.colours(d.data.series))
        ...
        function selectPoints(rect) {
            ...
            node.each(function (d, i) {
                ...
                else if (!that.selectedNodes.has(d.data.name)) {
                    n.classed("selected", false);
                    n.select("circle.node-circle").style("fill", "#FFF");
                }
            });
        ...
        function updateSelection() {
            ...
            that.svg.selectAll("g.node.selected").each(d => {
                if (!that.selectedNodes.has(d.data.name)) {
                    that.selectedNodes.add(d.data.name);
                    wasChange = true;
                }
            });
            ...
            that.svg.selectAll("g.node:not(.selected)").each(d => {
                if (that.selectedNodes.has(d.data.name)) {
                    that.selectedNodes.delete(d.data.name);
                    wasChange = true;
                }
            });

            if (wasChange && scope.selectedNodes) {
                scope.selectedNodes = Array.from(that.selectedNodes);
                scope.$apply();
            }
        }
...
function LineagePlotDirectiveX($window, WindowResize) {
    return {
        restrict: 'EA',
        scope: {
            value: '=',
