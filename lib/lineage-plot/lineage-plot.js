...
class LineagePlotController {
    ...
    render(options) {
        ...
        if (isTimePlot) {
            newDomain[0] = new Date(originalExtent[0].getTime() - labelExtraSpace)
        }
        else if(labelExtraSpace > 1) {
            newDomain[0] = originalExtent[0] - labelExtraSpace;
        }
        if (isTimePlot) {
            newDomain[1] = new Date(originalExtent[1].getTime() + labelExtraSpace)
        }
        ...
        if (layout.axis.show) {
            ...
            console.log(Math.ceil(xScale.domain().reduce((a, b) => b - a)));
