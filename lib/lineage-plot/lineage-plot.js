...
import { d3tooltip, mergeTemplateLayout, createNodeTypes, labelCollisionDetection, createTreeLayout, spreadGenerations,
    createDynamicNodeAttr, scaleProperties, getNodeLabelBBox, drawColourBar } from '../shared-features.js'
            ...
            function render(options) {
                ...
                if (layout.heatmap.enabled) {
                    heatmapColourScale = d3.scale.linear()
                        .domain(d3.extent(treeData, node => node.z))
                ...
                if (layout.heatmap.enabled) {
                    var heatmapCircle = treeArea.append("g")
                        .attr("class", "heatmap-layer")
                        .selectAll("circle.heatmap-circle")
                        .data(d => d.nodes.filter(n => n.name != "virtualRoot"&& !isNaN(n.z)))
                        .enter()
                        .append("circle")
