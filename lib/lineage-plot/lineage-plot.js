...
class LineagePlotController {
    ...
    render(options) {
        ...
        let isTimePlot = layout.axis.valueProperty === "date";
        ...
        let generationExtent = d3.extent(treeData, node => isTimePlot ? new Date(node.date * 1000) : node.generation),
            originalExtent = angular.copy(generationExtent),
            treeWidth = width, spaceRight = 1;

        if (isTimePlot) {
            let [secondStart, secondEnd] = generationExtent.map(d => d.getTime() / 1000),
                secondDiff = secondEnd - secondStart,
                intervalInSeconds = layout.minTimeWidth.intervalInSeconds,
                rangeInPixels = layout.minTimeWidth.rangeInPixels;
            if (intervalInSeconds * width / secondDiff < rangeInPixels) {
                treeWidth = secondDiff / intervalInSeconds * rangeInPixels;
            }
        }
        else {
        ...
        // define x scale
        let xScale = (isTimePlot ? d3.scaleTime() : d3.scaleLinear())
        ...
        if (isTimePlot) {
            newDomain[0] = new Date(generationExtent[0].getTime() - labelExtraSpace)
        }
        else if(labelExtraSpace > 1) {
            newDomain[0] = originalExtent[0] - labelExtraSpace;
        }
        if (isTimePlot) {
            newDomain[1] = new Date(generationExtent[1].getTime() + labelExtraSpace)
        }
        else if (labelExtraSpace > spaceRight) {
        ...
        if (layout.axis.show) {
            ...
            if (!isTimePlot) xAxis.ticks(Math.ceil(xScale.domain().reduce((a, b) => b - a)));
        }
        ...
        descendants.forEach(node => {
            ...
            node.x = xScale(isTimePlot ? new Date(node.data.date * 1000) : node.data.generation) ;
        });
        ...
        function applyZoom(zoomTransform) {
            ...
            if(!isTimePlot) xAxis.ticks(Math.ceil(xScale.domain().reduce((a, b) => b - a) / scale));
