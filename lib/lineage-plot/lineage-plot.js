...
function plotTree(node, nds, currentBranch=0) {
    if(!node.children.length) {console.log("creating key " + currentBranch);
    ...
    for(let child of node.children) {
        ...
        [branchLevel, currentBranch] = plotTree(child, nds, currentBranch);
    ...
    let meanBranchLevel = d3.extent(branchLevels).reduce((a, b) => a + b) / 2;
    console.log("Adding to branches: " + addToBranches);
    for(let b of addToBranches) {
