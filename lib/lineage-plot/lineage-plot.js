...
import d3 from 'd3'
import { d3tooltip, mergeTemplateLayout, createNodeTypes, labelCollisionDetection,
    createTreeLayout, spreadGenerations, createDynamicNodeAttr, scaleProperties, getNodeLabelBBox } from '../shared-features.js'
    ...
    return {
        ...
        link(scope, element, attributes) {
            ...
            function render(options) {
                ...
                let maxNodeLabelLength = d3.max(label.data().map(d => d.bbox.width)),
                    searchRadius = 2 * maxNodeLabelLength;

                if (layout.labelCollisionDetection === "onEveryChange" || layout.labelCollisionDetection === "onInit" ||
                    layout.labelCollisionDetection === "onDelay") {
                    let nodes = treesData[0].nodes.slice(0, -1);
                    LCD = new labelCollisionDetection(nodes, labelPositions, layout.nodeLabel, (x) => x, (y) => y,
                ...
                function applyZoom() {
                    ...
                    if (layout.labelCollisionDetection === "onInit" || layout.labelCollisionDetection === "onDelay" ||
                        layout.labelCollisionDetection === false) {
